...
then
  echo;
  echo "  .---------------------------.";
  echo "  | Monitor wird gestartet... |";
  echo "  '---------------------------'";
else
  $IPT -N $CHAIN
  echo;
  echo IPtables-Chain $CHAIN wurde angelegt!
  echo;
  echo;
  echo "  .---------------------------.";
  echo "  | Monitor wird gestartet... |";
...
echo "Folgende IP-Adressen wurden am `date` gesperrt:" > $BANNED_IP_MAIL
echo >>    $BANNED_IP_MAIL
#BAD_IP_LIST=`$TMP_FILE`
...
if [ $BAN -eq 1 ]; then
    ...
    while read line; do
        ...
        IP_BAN_NOW=1
        echo "$CURR_LINE_IP mit $CURR_LINE_CONN SYN_RECV Verbindungen" >> $BANNED_IP_MAIL
    ...
    if [ $IP_BAN_NOW -eq 1 ]; then
        dt=`date`
                hn=`hostname`
        if [ $EMAIL_TO != "" ]; then
            cat $BANNED_IP_MAIL | mail -s "IP-Adresse gebannt am $DATUM" $EMAIL_TO
        fi
        unbanip
    fi
fi
...
echo;
echo "Synblock hat bereits $summe Angreifer abgewehrt";
echo;
echo "|Eingehende SYN-Verbindungen|"
autoban
done;
